...
function attachCandidatesForTarget(targetStub, treePoints, treeSegments, preferredSegments = [], options = {}) {
  ...
  entries.forEach((entry) => {
    ...
    if (!current || entry.priority < current.priority) byKey.set(key, entry);
  });
...
function orthogonalSegmentIntersectsObstacle(a, b, bound, clearance = 6) {
...
function segmentHitsObstacles(a, b, obstacles, clearance = 6) {
  return obstacles.some((bound) => orthogonalSegmentIntersectsObstacle(a, b, bound, clearance));
}

function validateOrthogonalPath(points, obstacles, clearance = 6) {
...
function horizontalClear(y, x1, x2, obstacles) {
  return !segmentHitsObstacles({ x: x1, y }, { x: x2, y }, obstacles, 6);
}

function verticalClear(x, y1, y2, obstacles) {
  return !segmentHitsObstacles({ x, y: y1 }, { x, y: y2 }, obstacles, 6);
}
...
function buildCorridorCandidates(sourceStub, targetStub, sceneContext, trackUsage = new Map(), extraPoints = []) {
  const xChannels = new Map();
  const yChannels = new Map();
  addLaneVariants(xChannels, sourceStub.x, 72, { kind: "stub", priority: 10 });
  addLaneVariants(xChannels, targetStub.x, 72, { kind: "stub", priority: 10 });
  ...
  addLaneVariants(yChannels, sourceStub.y, 72, { kind: "stub", priority: 10 });
  addLaneVariants(yChannels, targetStub.y, 72, { kind: "stub", priority: 10 });
  ...
  extraPoints.forEach((point) => {
    addLaneVariants(xChannels, point.x, 72, { kind: "tree", priority: 24 });
    addLaneVariants(yChannels, point.y, 72, { kind: "tree", priority: 24 });
  });
  ...
  bounds.forEach((bound) => {
    addLaneVariants(xChannels, bound.left - 28, 72, { kind: "clearance-edge", priority: 14 });
    addLaneVariants(xChannels, bound.right + 28, 72, { kind: "clearance-edge", priority: 14 });
    addLaneVariants(yChannels, Math.max(24, bound.top - 28), 72, { kind: "clearance-edge", priority: 14 });
    addLaneVariants(yChannels, bound.bottom + 28, 72, { kind: "clearance-edge", priority: 14 });
  });
  ...
  for (let i = 0; i < localBands.length - 1; i += 1) {
    ...
    if (gap >= 40) addLaneVariants(yChannels, localBands[i].bottom + gap / 2, gap, { kind: "local-gap", priority: 42 });
  }
  ...
  for (let i = 0; i < localColumns.length - 1; i += 1) {
    ...
    if (gap >= 40) addLaneVariants(xChannels, localColumns[i].right + gap / 2, gap, { kind: "local-gap", priority: 42 });
  }

  for (let i = 0; i < byLeft.length - 1; i += 1) {
    const gap = byLeft[i + 1].left - byLeft[i].right;
    if (gap >= 64) addLaneVariants(xChannels, byLeft[i].right + gap / 2, gap, { kind: "inter-group", priority: 34 });
  }
  for (let i = 0; i < byTop.length - 1; i += 1) {
    const gap = byTop[i + 1].top - byTop[i].bottom;
    if (gap >= 64) addLaneVariants(yChannels, byTop[i].bottom + gap / 2, gap, { kind: "inter-group", priority: 34 });
  }
...
function buildCorridorGraph(sourceStub, targetStub, sceneContext, trackUsage, signalId, extraPoints = []) {
  const obstacles = [...sceneContext.objectBounds.values()];
  const { xs, ys, xMeta, yMeta } = buildCorridorCandidates(sourceStub, targetStub, sceneContext, trackUsage, extraPoints);
  ...
  rowMap.forEach((rowNodes, y) => {
    ...
    for (let i = 0; i < rowNodes.length - 1; i += 1) {
      ...
      const meta = yMeta.get(Math.round(y)) || { priority: 0 };
      const reservationPenalty = getReservedTrackPenalty(trackUsage, `hy:${Math.round(y)}`, signalId);
      ...
      let cost = Math.abs(a.x - b.x) + reservationPenalty + occupancyPenalty + bundleAdjustment;
      cost *= meta.priority >= 40 ? 0.74 : meta.priority >= 30 ? 0.84 : meta.priority >= 18 ? 0.94 : 1;
      if (meta.kind === "outer") cost += 60;
  ...
  colMap.forEach((colNodes, x) => {
    ...
    for (let i = 0; i < colNodes.length - 1; i += 1) {
      ...
      const meta = xMeta.get(Math.round(x)) || { priority: 0 };
      const reservationPenalty = getReservedTrackPenalty(trackUsage, `vx:${Math.round(x)}`, signalId);
      ...
      let cost = Math.abs(a.y - b.y) + reservationPenalty + occupancyPenalty + bundleAdjustment;
      cost *= meta.priority >= 40 ? 0.74 : meta.priority >= 30 ? 0.84 : meta.priority >= 18 ? 0.94 : 1;
      if (meta.kind === "outer") cost += 60;
...
function fallbackOrthogonalPath(sourceStub, targetStub, sceneContext) {
  ...
  if (candidates.length) {
    const winner = candidates.sort((a, b) => a.cost - b.cost).find((candidate) => validateOrthogonalPath(candidate.points, obstacles, 6));
  ...
  if (validateOrthogonalPath(fallback, obstacles, 6)) return fallback;
  return [
    sourceStub,
...
function shortestCorridorPathForSignal(sourceStub, targetStub, sceneContext, trackUsage, signalId, reserve = true) {
  const { nodeById, adjacency } = buildCorridorGraph(sourceStub, targetStub, sceneContext, trackUsage, signalId);
  ...
  if (compact.length >= 3) {
    ...
    if (reserve) reservePathTracks(simplified, trackUsage, signalId);
    return simplified;
  }
  ...
  if (reserve) reservePathTracks(fallback, trackUsage, signalId);
  return fallback;
}
...
function shortestCorridorPathToAny(startStub, destinationEntries, referenceStub, sceneContext, trackUsage, signalId, reserve = true) {
  ...
  const reference = referenceStub || destinationPoints?.[0] || startStub;
  const { nodeById, adjacency } = buildCorridorGraph(startStub, reference, sceneContext, trackUsage, signalId, extraPoints);
  const startId = `${Math.round(startStub.x)}:${Math.round(startStub.y)}`;
  const destinationPenalty = new Map(destinations.map((entry) => [`${Math.round(entry.point.x)}:${Math.round(entry.point.y)}`, entry.priority || 0]));
  const destinationIds = new Set(uniquePointList(destinationPoints).map((point) => `${Math.round(point.x)}:${Math.round(point.y)}`));
  if (!nodeById.has(startId) || !destinationIds.size) return null;
  ...
  dist.set(startKey, 0);
  queue.push({ nodeId: startId, dir: "none", cost: 0 });
  ...
  const bestTargetState = [...dist.keys()]
    .filter((key) => destinationIds.has(key.split("|")[0]))
    .sort((a, b) => {
      const aNode = a.split("|")[0];
      const bNode = b.split("|")[0];
      return (dist.get(a) + (destinationPenalty.get(aNode) || 0)) - (dist.get(b) + (destinationPenalty.get(bNode) || 0));
    })[0];
  if (!bestTargetState) return null;

  const points = [];
  let stateKey = bestTargetState;
  while (stateKey) {
    const [nodeId] = stateKey.split("|");
    const node = nodeById.get(nodeId);
    if (node) points.push({ x: node.x, y: node.y });
  ...
  points.reverse();
  const compact = uniquePointList(points);
...
function buildSignalRouteSegments(signal, sceneContext, layers, trackUsage, renderMetrics = null) {
  ...
  const routeObstaclesFor = (entry) => [...sceneContext.objectBounds.entries()]
    .filter(([id]) => id !== sourceObject.id && id !== entry.object.id)
    .map(([, bounds]) => bounds);
  ...
  const primaryCandidates = targetsWithStubs.map((entry) => {
    ...
    return {
      ...
      score: orthogonalPathCost(fullPath) + attachmentCost + flowPenalty - dominanceBonus
    };
  ...
  if (!validateOrthogonalPath(primaryPath, routeObstaclesFor(primary.entry), 6)) {
    ...
    if (validateOrthogonalPath(fallback, routeObstaclesFor(primary.entry), 6)) {
  ...
  remaining.forEach((entry) => {
    ...
    if (!branchPath || !validateOrthogonalPath(branchPath, routeObstaclesFor(entry), 6)) {
      ...
      if (validateOrthogonalPath(fallback, routeObstaclesFor(entry), 6)) {
        branchPath = fallback;
      }
    }
