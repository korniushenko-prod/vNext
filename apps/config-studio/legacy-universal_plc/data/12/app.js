...
function shortestCorridorPath(sourceStub, targetStub, sceneContext) {
  ...
  if (!nodeById.has(sourceId) || !nodeById.has(targetId)) return [sourceStub, targetStub];
  ...
  if (!targetStates.length) return [sourceStub, targetStub];
  ...
  return compact.length ? compact : [sourceStub, targetStub];
}
...
function buildSignalRouteSegments(signal, sceneContext, layers, trackUsage) {
  ...
  const sourceStub = { x: sourceAnchor.x + 28, y: sourceAnchor.y };
  ...
  validTargets.forEach((entry) => {
    const targetStub = { x: entry.anchor.x - 28, y: entry.anchor.y };
