...
function buildRoutingBoard(selectedSignal) {
  ...
  leftColumn.className = "routing-board-column";
  const rightColumn = document.createElement("div");
  rightColumn.className = "routing-board-column";
  ...
  objects().forEach((obj, index) => {
    ...
    column.append(renderObjectCard(obj));
  });
  ...
  requestAnimationFrame(() => {
    ...
    signals().forEach((signal) => {
      const sourceKey = `${signal.source.object_id}::${signal.source.port}`;
      const sourceDot = sourceDotRefs.get(sourceKey);
      if (!sourceDot) return;
    ...
    if (state.routeConnect.active) {
      const previewSource = sourceDotRefs.get(`${state.routeConnect.sourceObjectId}::${state.routeConnect.sourcePort}`);
