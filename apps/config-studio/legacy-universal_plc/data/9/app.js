...
function routeEdgePath(sourceAnchor, targetAnchor, trackIndex) {
  const stub = 28;
  const spacing = 18;
...
function buildRoutingBoard(selectedSignal) {
  ...
  requestAnimationFrame(() => {
    ...
    signals().forEach((signal) => {
      ...
      if (!sourceObject || !sourcePort) return;
      const sourceAnchor = portAnchor(sourceObject, sourcePort, "outputs", "right");
      (signal.targets || []).forEach((target) => {
        const targetKey = `${target.object_id}::${target.port}`;
        const targetDot = targetDotRefs.get(targetKey);
        if (!targetDot) return;
        const targetObject = findObjectById(target.object_id);
        const targetPort = targetObject ? objectPorts(target.object_id, "inputs").find((port) => port.name === target.port) : null;
        if (!targetObject || !targetPort) return;
        const targetAnchor = portAnchor(targetObject, targetPort, "inputs", "left");
        const srcLayer = Math.round((sourceObject.ui.routing.x - 120) / 420);
        const dstLayer = Math.round((targetObject.ui.routing.x - 120) / 420);
        const trackKey = `${srcLayer}->${dstLayer}`;
        const trackIndex = trackUsage.get(trackKey) || 0;
        trackUsage.set(trackKey, trackIndex + 1);
        const path = document.createElementNS("http://www.w3.org/2000/svg", "path");
        path.setAttribute("d", routeEdgePath(sourceAnchor, targetAnchor, trackIndex));
