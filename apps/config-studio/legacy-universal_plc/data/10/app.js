...
function chooseFamilyLane(sourceAnchor, entries, familyBounds, sceneContext, trackUsage, baseKey) {
  const spacing = 18;
  const pad = 42;
  ...
  const scored = candidates
    .map((candidate) => {
      ...
      const cost = ys.reduce((sum, value) => sum + Math.abs(value - y), 0) + candidate.penalty;
      return {
        key: candidate.key,
        kind: candidate.kind,
        y,
  ...
  if (gapCandidates.length) {
    ...
    best = (!bestOuter || bestOuter.cost > bestGap.cost - 120) ? bestGap : bestOuter;
  }
...
function buildSignalRouteSegments(signal, sceneContext, layers, trackUsage) {
  ...
  const sourceLayer = layers.get(sourceObject.id) || 0;
  const sourceLayerBounds = sceneContext.layerBounds.get(sourceLayer);
  if (!sourceLayerBounds) return [];
  ...
  const validTargets = (signal.targets || []).map((target) => {
    ...
    return {
      ...
      layer: layers.get(object.id) || 0,
      bounds: sceneContext.objectBounds.get(object.id)
    };
  }).filter(Boolean);

  if (!validTargets.length) return [];
  const forwardTargets = validTargets.filter((entry) => entry.layer > sourceLayer);
  const returnTargets = validTargets.filter((entry) => entry.layer <= sourceLayer);
  return [
    ...buildForwardGroupSegments(signal, sourceAnchor, sourceLayer, sourceLayerBounds, forwardTargets, sceneContext, trackUsage),
    ...buildReturnGroupSegments(signal, sourceAnchor, sourceLayerBounds, returnTargets, sceneContext, trackUsage)
  ].filter(Boolean);
}
